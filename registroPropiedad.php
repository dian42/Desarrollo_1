...
if (count($_SESSION) != 0 && $_SESSION['tipo'] == false ) {
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		if(isset($_POST['conjunto'], $_POST['numero'], $_POST['m2'],  $_POST['alicuota'])) {
			...
			if(Vpropiedad($numero) && Vm2($m2) && Valicuota($alicuota) && Vconjunto($conjunto,$conexion_bd)){
				...
				if ($largo){
					...
					render('Rpropiedad/errorNew.html.twig', array('conjunto' => $consult_propiedad, 'valido' => $_SESSION['valido'], 'error'=>"Propiedad existente, por favor intentelo nuevamente."));
				}
				else {
				...
				$con = $conexion_bd->prepare("SELECT * FROM propiedad");
				$con -> execute();
				$consult_propiedad = $con -> fetchAll(PDO::FETCH_ASSOC); //Saca el todas las propiedades asociadas a un conjunto
				render('Rpropiedad/errorNew.html.twig', array('conjunto' => $consult_propiedad, 'valido' => $_SESSION['valido'], 'error'=>"Valores ingresados no válidos"));
